...
class FacesFuckApp:
    ...
    def setup_ui(self):
        ...
        tk.Label(self.nav_bar, text="FacesFuck STUDIO", fg="#cba6f7", bg="#181825", font=("Segoe UI", 18, "bold")).pack(side="left", padx=30)
        tk.Button(self.nav_bar, text="📂 VIEW CREATIONS", command=self.open_output_folder, bg="#313244", fg="#cdd6f4", font=("Segoe UI", 9, "bold"), relief="flat", activebackground="#45475a", cursor="hand2", padx=20, pady=8).pack(side="right", padx=30)
        ...
        tk.Label(self.status_container, textvariable=self.status_var, bg="#0f0f14", fg="#9399b2", font=("Segoe UI", 11)).pack()
        ...
        self.gen_btn = tk.Button(self.btn_container, text="START GENERATION", command=self.start_processing, bg="#a6e3a1", fg="#11111b", font=("Segoe UI", 12, "bold"), relief="flat", padx=40, pady=15, cursor="hand2", activebackground="#94e2d5")
        self.cancel_btn = tk.Button(self.btn_container, text="CANCEL PROCESS", command=self.confirm_cancel, bg="#f38ba8", fg="#11111b", font=("Segoe UI", 12, "bold"), relief="flat", padx=40, pady=15, cursor="hand2")
    ...
    def create_drop_zone(self, title, col):
        ...
        btn = tk.Button(f, text="Choose File", bg="#45475a", fg="#cdd6f4", relief="flat", font=("Segoe UI", 9), command=lambda t=file_type: self.browse_file(t), activebackground="#585b70", cursor="hand2")
        btn.pack(pady=5)
        thumb = tk.Label(f, text="\n\nDrag & Drop\nFile Here", bg="#181825", fg="#585b70", width=35, height=12, font=("Segoe UI", 10))
    ...
    def toggle_inputs(self, state):
        ...
        self.img_btn.config(state=tk_state); self.vid_btn.config(state=tk_state)
        alpha = "#1e1e2e" if state else "#11111b"
        self.img_panel.config(bg=alpha); self.vid_panel.config(bg=alpha)
        if state:
            self.img_panel.drop_target_register(DND_FILES); self.vid_panel.drop_target_register(DND_FILES)
        else:
            self.img_panel.drop_target_unregister(); self.vid_panel.drop_target_unregister()
    ...
    def confirm_cancel(self):
        ...
        tk.Label(self.toast, text="Abort Processing?", fg="#f38ba8", bg="#181825", font=("Segoe UI", 14, "bold")).pack(pady=(30, 10))
        tk.Label(self.toast, text="All progress will be lost.", fg="#a6adc8", bg="#181825", font=("Segoe UI", 10)).pack(pady=(0, 20))
        bf = tk.Frame(self.toast, bg="#181825"); bf.pack()
    ...
    def reset_app(self):
        ...
        self.source_path = None; self.video_path = None
        self.thumb_img.config(image='', text="\n\nDrag & Drop\nFile Here", bg="#181825")
        self.thumb_vid.config(image='', text="\n\nDrag & Drop\nFile Here", bg="#181825")
        self.thumb_img.image = None; self.thumb_vid.image = None
        self.img_panel.config(highlightbackground="#313244"); self.vid_panel.config(highlightbackground="#313244")
        self.gen_btn.pack_forget(); self.cancel_btn.pack_forget(); self.progress.pack_forget()
        self.status_var.set("Ready to Swap"); self.toggle_inputs(True)

    def open_output_folder(self):
        os.startfile(self.output_dir) if os.name == 'nt' else subprocess.call(['open', self.output_dir])

    def browse_file(self, type):
        if type == "img": f = filedialog.askopenfilename(filetypes=[("Image", "*.jpg *.jpeg *.png *.webp")])
        else: f = filedialog.askopenfilename(filetypes=[("Video", "*.mp4 *.avi *.mov *.mkv")])
    ...
    def load_file(self, path, type):
        ...
        if type == "img":
            ...
            self.source_path = path; self.update_thumbnail(path, self.thumb_img)
            self.img_panel.config(highlightbackground="#cba6f7")
        else:
            if ext not in ['.mp4', '.avi', '.mov', '.mkv']: return
            self.video_path = path; self.update_thumbnail(path, self.thumb_vid, is_video=True)
            self.vid_panel.config(highlightbackground="#cba6f7")
        if self.source_path and self.video_path: self.gen_btn.pack(pady=10)
    ...
    def start_processing(self):
        ...
        self.toggle_inputs(False); self.gen_btn.pack_forget(); self.cancel_btn.pack(); self.progress.pack(pady=10); self.progress['value'] = 0
    ...
    def process_video(self):
        try:
            ...
            if pts_src is None: raise Exception("No face found!")
            hull_src = cv2.convexHull(pts_src); rect = cv2.boundingRect(hull_src); subdiv = cv2.Subdiv2D(rect)
            for p in pts_src: subdiv.insert((float(p[0]), float(p[1])))
            triangles = subdiv.getTriangleList(); idx_tri = []
            for t in triangles:
                pt1, pt2, pt3 = (t[0], t[1]), (t[2], t[3]), (t[4], t[5])
                i1 = np.where((pts_src == pt1).all(axis=1))[0]; i2 = np.where((pts_src == pt2).all(axis=1))[0]; i3 = np.where((pts_src == pt3).all(axis=1))[0]
                if i1.size > 0 and i2.size > 0 and i3.size > 0: idx_tri.append([i1[0], i2[0], i3[0]])
            cap = cv2.VideoCapture(self.video_path); fps = cap.get(cv2.CAP_PROP_FPS); w, h = int(cap.get(3)), int(cap.get(4)); total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
            out = cv2.VideoWriter(temp_silent, cv2.VideoWriter_fourcc(*'mp4v'), fps, (w, h))
            count = 0; prev_pts_dst = None
            while cap.isOpened() and self.is_processing:
                ...
                if curr_pts_dst is not None:
                    ...
                    prev_pts_dst = pts_dst; img_source = color_transfer(img_src_raw, frame); canvas_warped = np.zeros_like(frame)
                    for tri_idx in idx_tri:
                        ...
                        if img_s_c.shape[0] > 0 and img_s_c.shape[1] > 0:
                            ...
                            mask = np.zeros((r_d[3], r_d[2], 3), dtype=np.uint8); cv2.fillConvexPoly(mask, dtc, (255, 255, 255))
                            roi = canvas_warped[r_d[1]:r_d[1]+r_d[3], r_d[0]:r_d[0]+r_d[2]]
                            if roi.shape[0] == warp.shape[0] and roi.shape[1] == warp.shape[1]:
                                canvas_warped[r_d[1]:r_d[1]+r_d[3], r_d[0]:r_d[0]+r_d[2]] = cv2.bitwise_and(roi, cv2.bitwise_not(mask)) + cv2.bitwise_and(warp, mask)
                    hull_dst = cv2.convexHull(pts_dst); mask_dst = np.zeros_like(frame); cv2.fillConvexPoly(mask_dst, hull_dst, (255, 255, 255))
                    mask_dst = cv2.GaussianBlur(cv2.erode(mask_dst, np.ones((5,5), np.uint8)), (11, 11), 0)
                    try: frame = cv2.seamlessClone(canvas_warped, frame, mask_dst, (cv2.boundingRect(hull_dst)[0] + cv2.boundingRect(hull_dst)[2] // 2, cv2.boundingRect(hull_dst)[1] + cv2.boundingRect(hull_dst)[3] // 2), cv2.NORMAL_CLONE)
                    except: pass
                out.write(frame); count += 1; self.progress['value'] = (count / total_frames) * 100
                self.status_var.set(f"Processing: {count}/{total_frames}"); self.root.update_idletasks()
            ...
            if os.path.exists(temp_silent): os.remove(temp_silent)
            messagebox.showinfo("Complete", f"Saved to Desktop/FacesFuck"); self.reset_app()
        except Exception as e:
            messagebox.showerror("Error", str(e)); self.reset_app()
